...
처음에는 `App.tsx`의 아래 값을 그대로 두면 기존 로컬 저장 목업으로만 동작합니다.
...
6. `App.tsx`에서 `USE_FIREBASE = true`로 변경
7. 실제 기기에서 위치 권한, 알림 권한, 도착/퇴장 저장 흐름 재검증
...
`src/services/firebase.ts`는 모든 `EXPO_PUBLIC_FIREBASE_*` 값이 채워진 경우에만 Firebase를 초기화합니다. 값이 비어 있으면 `db`는 `null`이고, 앱은 기존 로컬 저장 목업을 계속 사용합니다.
...
- Firebase 모드 전환 시 `USE_FIREBASE = true`로 변경
- 로그인 붙인 뒤 `USER_ID`를 실제 인증 uid로 교체
